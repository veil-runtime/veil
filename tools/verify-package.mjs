...
assert.ok(!files.some(
  (path) => path.startsWith('dist/providers/')
    && !path.startsWith('dist/providers/storage/sqlite-job-store.'),
));

cpSync(join(root, 'test', 'fixtures', 'package-consumer'), fixtureDirectory, { recursive: true });
execFileSync(
  process.execPath,
  [
    npmCli,
...
execFileSync(
  process.execPath,
  [join(root, 'node_modules', 'typescript', 'bin', 'tsc'), '--project', join(fixtureDirectory, 'tsconfig.json')],
  { cwd: fixtureDirectory, stdio: 'inherit' },
);

execFileSync(process.execPath, [join(fixtureDirectory, 'verify.mjs')], {
