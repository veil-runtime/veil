...
const ALLOWED_ROOT = resolve(
  process.env.OPERATOR_FILES_ROOT ?? process.cwd()
);

function resolveSafePath(inputPath: string): string {
  const candidate = isAbsolute(inputPath)
    ? resolve(inputPath)
    : resolve(ALLOWED_ROOT, inputPath);

  const rel = relative(ALLOWED_ROOT, candidate);
...
export const filesystemFileReadCapability: Capability<
  FileReadInput,
  FileReadResult
> = {
  ...
  async execute(input, context) {
    ...
    const path = resolveSafePath(input.path);
