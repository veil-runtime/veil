...
`npm run mcp:stdio` starts the repository stdio server with built-in capabilities. The adapter does not pass MCP caller identity into `executePlan` in v0.1.3, so an authorizer cannot identify an MCP caller through `ExecutionCaller` without an application-owned wrapper.
